...
for target in target_list:
    ...
    targets[target]['half_target'] = config_parser[target].getboolean(
        'HALF_TARGET')
    targets[target]['offset_direction'] = config_parser[target]['OFFSET_DIRECTION'].lower()
    targets[target]['target_color'] = tuple(
        map(int, config_parser[target]['TARGET_COLOR'].split()))
    targets[target]['found_center_color'] = tuple(
        map(int, config_parser[target]['FOUND_CENTER_COLOR'].split()))
...
while True:
    ...
    thresh = cv2.inRange(cv2.cvtColor(
        frame, cv2.COLOR_BGR2HSV), min_color, max_color)
    ...
    lines = cv2.HoughLinesP(edges, 1, np.pi/360, 20,
            ...
            for x1, y1, x2, y2 in unpacked_line:
                ...
                for target_name in targets:
                    ...
                    for angle_name in target['angle_names']:
                        ...
                        if return_value[1]:
                            target[angle_name]['valid_points']['x'] = [
                                x1, x2] + target[angle_name]['valid_points']['x'][:18]
                            target[angle_name]['valid_points']['y'] = [
                                y1, y2] + target[angle_name]['valid_points']['x'][:18]
    ...
    for target_name in targets:
        ...
        if all(target[angle_name]['present'] for angle_name in target['angle_names']):
            [item for angle_name in target['angle_names']
                for item in target[angle_name]['valid_points']['x']]
            if verbosity >= 2:
                print(f'{target_name} found.')
            x_vals = [item for angle_name in target['angle_names']
                      for item in target[angle_name]['valid_points']['x']]
            x_avg = int(sum(x_vals)/len(x_vals))
            y_vals = [item for angle_name in target['angle_names']
                      for item in target[angle_name]['valid_points']['y']]
            ...
            if verbosity >= 1:
                ...
                cv2.circle(black, tuple(map(abs, target_position)),
                           15, target['found_center_color'])
